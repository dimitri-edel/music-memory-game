...
class MP3Player {
    ...
    play = (n) => {
        ...
        if (n !== null) {
            ...
            // 
            this.audio = new Audio(this.audio_files_foler_path + this.tracks[n].filename);
            this.audio.volume = this.volume;
            this.audio.play();
        } else {
            // if the audio has been stoppped, load the audio file and play it
            if (this.audio == null) {
                this.audio = new Audio(this.audio_files_foler_path + this.tracks[this.trackIndex].filename);
                this.audio.volume = this.volume;
                this.audio.play();
            } else {
